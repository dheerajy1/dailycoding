    ...
    <header className="bg-(--food-bg-surface) shadow-sm px-6 py-4 flex justify-between items-center sticky top-0 z-10">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden sm:flex items-center text-sm font-medium text-(--food-text-muted)">
          📍 Delivering to:{" "}
          <span className="text-(--food-text-dark) ml-1">Khammam, TG</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={onMenuClick}
          className="relative p-2 text-(--food-text-muted) hover:bg-(--food-border-light) rounded-full transition-colors"
        >
          🛒
          {cartCount > 0 && (
            <span className="absolute top-0 right-0 bg-(--food-accent-brand) text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full font-bold animate-pulse">
              {cartCount}
            </span>
          )}
        </button>
